...
export class ListFarmingRateComponent implements OnInit {
    ...
    //#region Sorting
    sortData(sort: any) {
        this.farmingRateSort[0] = sort.active;
        this.farmingRateSort[1] = sort.direction;
        this._customerService.getFarmingRate(
            this.routeID,
            this.farmingRateSort[0],
            this.farmingRateSort[1],
            this.searchResult
        );
    }
    //#endregion

    //#region Confirmation Customer Farming Rate Dialog
    confirmDeleteDialog(id: string): void {
        const dialogRef = this._matDialog.open(ConfirmationDialogComponent, {
            data: {
                message: 'Are you sure you want to delete this Farming Rate?',
                title: 'Farming Rate',
            },
        });
