...
export class ListHaulingRateComponent implements OnInit {
    ...
    //#region Sorting
    sortData(sort: any) {
        this.haulingRateSort[0] = sort.active;
        this.haulingRateSort[1] = sort.direction;
        this._customerService.getHaulingRate(
            this.routeID,
            this.haulingRateSort[0],
            this.haulingRateSort[1],
            this.searchResult
        );
    }
    //#endregion

    //#region Confirmation Customer Hauling Rate Dialog
    confirmDeleteDialog(id: string): void {
        const dialogRef = this._matDialog.open(ConfirmationDialogComponent, {
            data: {
                message: 'Are you sure you want to delete this Hauling Rate?',
                title: 'Hauling Rate',
            },
        });
